    ...
    def handle(self, *args, **options):
        for arg in options['filenames']:
            ...
            if schema == 'complete' or schema == 'league':
                league_info, teams_info = sports.League.import_league(data)
                self.stdout.write('{} league {}\n'.format(
                    'Created' if league_info[1] else 'Updated',
                    league_info[0]
                ))
            ...
            if schema == 'complete' or schema == 'season':
                results = sports.League.import_season(data)
                count = len(results)
                created = sum(1 for r in results if r[1])
                print('Processed {} games: {} new, {} updated'.format(
                    count,
                    created,
                    count - created
                ))
